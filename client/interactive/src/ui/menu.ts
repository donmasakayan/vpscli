  ...
  while (true) {
    ...
    console.log(
      chalk.bold(`  ${machine.name}`) +
        chalk.dim(` · ${vpscliUser} · v${version.current}`) +
        updateNotice,
    );
    ...
    if (staleSessions.length > 0) {
      ...
      if (!p.isCancel(cleanUp) && cleanUp) {
        for (const s of staleSessions) {
          const { exitCode } = await sshExec(machine, `bash ~/.vpscli/session.sh end '${s.name}'`);
          if (exitCode === 0) {
    ...
    options.push({
      ...
      hint: hasUpdate ? `v${version.latest} available` : "fetch latest vpscli + machines",
    });
    ...
    if (typeof action === "string" && action.startsWith("resume:")) {
      ...
      await cmdStart(machine, sessionName, vpscliUser);
      break;
    }
    ...
    switch (action) {
      case "new-session": {
        ...
        await cmdStart(machine, name, vpscliUser);
        break;
      }
      case "join-team": {
        ...
        await cmdStart(machine, choice, vpscliUser);
        break;
      }
      case "manage":
        await cmdSessions(machine, vpscliUser);
        break;
      case "setup":
