    ...
    const handleAction = async (action: AppAction) => {
      try {
        ...
        switch (action.type) {
          ...
          case "new-session":
          case "join-team":
            await cmdStart(machine, action.sessionName, vpscliUser, undefined, undefined, moshOpts);
            resolve();
            break;
          case "new-session-in-repo":
            await cmdStart(machine, action.sessionName, vpscliUser, undefined, action.repoPath, moshOpts);
            resolve();
            break;
          case "cd-to-repo": {
            const envCmd = `source ~/.vpscli/developers/${vpscliUser}.env 2>/dev/null; `;
