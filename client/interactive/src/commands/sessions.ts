...
export async function cmdSessions(machine: Machine, vpscliUser: string): Promise<void> {
  ...
  while (true) {
    ...
    if (action === "join") {
      ...
      const cmd = `bash ~/.vpscli/session.sh start '${choice}' 'shell' '' '${vpscliUser}'`;
    ...
    if (action === "end") {
      ...
      const mine = sessions.filter((s) => s.owner === vpscliUser);
      ...
      for (const name of selected) {
        const { exitCode } = await sshExec(machine, `bash ~/.vpscli/session.sh end '${name}'`);
        if (exitCode === 0) {
