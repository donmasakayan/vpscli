...
export async function fetchSessions(machine: Machine): Promise<SessionData[]> {
  const { stdout, exitCode } = await sshExec(machine, "bash ~/.vpscli/session.sh session-data");
  if (exitCode !== 0) {
...
export async function fetchRepos(machine: Machine): Promise<RepoData[]> {
  const { stdout, exitCode } = await sshExec(machine, "bash ~/.vpscli/session.sh repo-list");
  if (exitCode !== 0) {
...
export async function fetchRepoCommits(machine: Machine, repoPath: string): Promise<CommitData[]> {
  const { stdout, exitCode } = await sshExec(machine, `bash ~/.vpscli/session.sh repo-commits '${repoPath}' 15`);
  if (exitCode !== 0) {
