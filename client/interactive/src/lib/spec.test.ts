...
describe("spec", () => {
  test("finds command specs by spaced path", () => {
    expect(findCommandSpec("repo")?.path).toBe("repo");
  });

  test("finds command specs by slash path", () => {
    expect(findCommandSpec("spec/command")?.path).toBe("spec command");
  });
  ...
  test("builds discoverability documents", () => {
    ...
    expect(capabilities.top_level_nouns).toContain("status");
  });
});
