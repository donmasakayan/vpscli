...
# All developers share the same tmux sessions via the current VPS account.
# Usage: session.sh <command> [args...]
# Commands: start, end, list, describe, reconcile, registry
# Developer identity: pass dev name as last arg to 'start' to source ~/.vpscli/developers/<name>.env

set -euo pipefail

REGISTRY="$HOME/.vpscli/sessions.yaml"
TMUX_CONF="$HOME/.vpscli/tmux.conf"
DEVELOPERS_DIR="$HOME/.vpscli/developers"
PATH="$HOME/.local/bin:$HOME/.bun/bin:$HOME/.volta/bin:$PATH"

# Ensure registry exists
if [[ ! -f "$REGISTRY" ]]; then
  mkdir -p "$(dirname "$REGISTRY")"
  printf '# VPS session registry -- managed by .vpscli/session.sh\nsessions: []\n' > "$REGISTRY"
fi
...
cmd_start() {
  ...
  # Use start_dir if provided, otherwise default to HOME
  local work_dir="$HOME"
  ...
  if [[ -n "$dev_user" ]] && [[ -f "$DEVELOPERS_DIR/${dev_user}.env" ]]; then
    local session_gitconfig="$HOME/.vpscli/developers/${dev_user}.gitconfig"
    printf '[user]\n\tname = %s\n\temail = %s\n' \
    ...
    if ! tmux_running "$sname"; then
      ...
      if [[ "$current_owner" == "unknown" ]]; then
        ...
        if [[ -n "$real_owner" ]]; then
          # Lowercase the owner name to match vpscli user convention
