...
curl -fsSL https://raw.githubusercontent.com/donmasakayan/vpscli/main/server/setup.sh | sudo bash
```
...
- Clones the project repo to `~/ubuntu`
...
├── session.sh      # Tmux session manager (installed to ~/.vpscli/)
└── tmux.conf       # Tmux config with resurrect+continuum (installed to ~/.vpscli/)
```

## Session Manager

The session manager runs on the VPS at `~/.vpscli/session.sh`. It's called by `vpscli` over SSH — you rarely need to use it directly.
...
If `vpscli sync-keys` isn't available:
