#!/usr/bin/env bash
# Auto-update vpscli binary on this VPS when a new GitHub release is available.
# Intended to run via cron every few minutes.
# Uses public GitHub API — no auth needed for public repos.
set -euo pipefail

BINARY="/home/ubuntu/.local/bin/vpscli"
REPO="donmasakayan/vpscli"
ASSET="vpscli-linux-x64"
LOG="/tmp/vpscli-auto-update.log"
...
echo "$(date -Iseconds) Updating vpscli: $CURRENT → $LATEST" >> "$LOG"
...
if curl -fsSL "$DOWNLOAD_URL" -o "$TMP"; then
  chmod +x "$TMP"
  mv "$TMP" "$BINARY"
  echo "$(date -Iseconds) Updated vpscli to $LATEST" >> "$LOG"
else
  rm -f "$TMP"
