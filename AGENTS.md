...
─────────────────────────────        ─────────────────────────────
~/.local/bin/vpscli (wrapper script)  ~/.vpscli/session.sh
~/.vpscli/bin/vpscli-core (binary)     ~/.vpscli/tmux.conf
~/.vpscli/config (VPSCLI_USER)         ~/.vpscli/sessions.yaml (registry)
~/.vpscli/machines.yaml               ~/.vpscli/developers/<name>.env
~/.ssh/vps/id_<name>_ed25519         ~/.vpscli/git-credential-token
                                     ~/.vpscli/hooks/notify-attach.sh
...
```
~/.local/bin/vpscli          (bash wrapper)
  └─ ~/.vpscli/bin/vpscli-core  (compiled Bun binary)
       └─ Ink TUI renders, user picks session
       └─ writes SSH command to $VPSCLI_EXEC_FILE, exits 10
...
**How it works**:
1. The wrapper script sets `VPSCLI_EXEC_FILE=/tmp/vpscli-exec-$$` and runs `vpscli-core`
2. `vpscli-core` renders the Ink TUI, user selects a session
3. The SSH command is written to `$VPSCLI_EXEC_FILE` and the process exits with code 10
...
**Auto-bootstrap**: On first client run without a wrapper (`VPSCLI_EXEC_FILE` not set, `~/.vpscli/bin/vpscli-core` doesn't exist), the binary copies itself to `vpscli-core`, writes the wrapper script to `~/.local/bin/vpscli` (atomic write via tmp + rename), and code-signs on macOS. The current invocation continues as the binary (fallback path), and subsequent runs go through the wrapper.

### Client (`client/`)

| File | Purpose |
|------|---------|
| `client/vpscli` | Legacy bash CLI (kept for fallback). |
| `client/interactive/` | Bun/TypeScript interactive CLI. Compiled to standalone binary via `bun build --compile`. |
| `client/interactive/src/index.ts` | Entry point: arg parsing → dashboard or subcommand dispatch. Runs `ensureWrapperInstalled()` early to auto-bootstrap wrapper + vpscli-core on first run. Handles `vpscli -` (resume last), `vpscli .` (branch session), first-run inline setup, VPS-native detection. |
...
| `client/setup.sh` | Idempotent installer. Downloads compiled binary to `~/.vpscli/bin/vpscli-core`, installs wrapper script to `~/.local/bin/vpscli` (with `stty sane` + exec handoff), code-signs on macOS. Falls back to GitHub releases when no local build. |
...
| File | Purpose |
|------|---------|
| `machines.yaml` | VPS registry. Parsed by `vpscli` via `awk`. One machine = auto-select, multiple = interactive prompt or `--machine` flag. |
| `keys/*.pub` | One SSH public key per engineer. Synced to VPS `authorized_keys` via `vpscli sync-keys` or `server/setup.sh`. |

## Key Design Decisions

- **Single shared user (`dev`)**: Enables tmux session sharing. Two devs running `vpscli feature-x` attach to the same terminal.
- **Per-developer identity**: `~/.vpscli/developers/<name>.env` sets `GIT_AUTHOR_NAME`, `GIT_AUTHOR_EMAIL`, `GIT_COMMITTER_NAME`, `GIT_COMMITTER_EMAIL`, `GH_TOKEN`. Sourced by `session.sh` at session start.
- **Git credential helper**: `~/.vpscli/git-credential-token` reads `GH_TOKEN` from the session environment and passes it to git for HTTPS auth. Configured via `git config --global credential.helper`.
- **Self-contained client**: After `vpscli update` or `client/setup.sh`, the repo clone is not needed. `vpscli` is a bash wrapper that execs `vpscli-core` (compiled Bun binary). The wrapper handles stdin handoff to avoid Bun/SSH contention (see **Wrapper + Binary Architecture** above). Auto-bootstraps on first run if only the bare binary is installed.
- **Version check**: On every run, `vpscli` checks GitHub for a newer version and warns the user.
- **Smart dashboard**: Running `vpscli` with no args on a TTY shows an Ink-based TUI with dual themes (teal on VPS, mauve remote), navigable button bar, full-width session highlights, responsive layout (wide/normal/narrow), and tab-based focus zones. Non-TTY falls back to quick shell. `--old` flag uses the legacy text menu.
- **VPS-native mode**: When `~/.vpscli` exists, `vpscli` runs commands locally (no SSH to self) and uses `tmux switch-client` instead of `attach-session` when already inside tmux.
- **Shortcuts**: `vpscli -` resumes the most recent own session. `vpscli .` creates/attaches a session named after the current git branch.
- **First-run inline setup**: If `VPSCLI_USER` is not set, `vpscli` launches setup interactively instead of showing an error.
...
- **CI workflow** (`.github/workflows/release.yml`):
  ...
  2. Builds 3 binaries: `vpscli-linux-x64`, `vpscli-darwin-arm64`, `vpscli-darwin-x64`
...
- Users get updates via `vpscli update`
...
The `vpscli` CLI currently only creates `shell` sessions. `claude` and `codex` types are available via direct `session.sh` invocation on the VPS.

## CLI Commands

| Command | Description |
|---------|-------------|
| `vpscli` | Interactive dashboard (TTY) or quick shell (non-TTY) |
| `vpscli <name> [desc]` | Create or attach to named session |
| `vpscli -` | Resume most recent own session |
| `vpscli .` | Session from current git branch name |
| `vpscli list` | List sessions (grouped: Active/Idle/Stale) |
| `vpscli end <name>` | End a session |
| `vpscli setup` | Configure developer identity |
| `vpscli sync-keys` | Sync SSH keys from repo to VPS |
| `vpscli update` | Update binary + machines from GitHub |
...
- **`$REPO_DIR` fallback**: `session.sh` must fall back to `$HOME` when `~/ubuntu` doesn't exist. Without this, `cd` fails and the tmux session dies instantly.
...
- **GitHub raw CDN**: Caches for several minutes after push. `vpscli update` right after pushing may fetch stale content.
...
- **Per-developer gitconfig isolation**: `session.sh` creates `~/.vpscli/developers/<name>.gitconfig` per developer and sets `GIT_CONFIG_GLOBAL` env var. This prevents concurrent sessions from overwriting each other's `git config` identity. Never run `git config --global user.name/email` directly on the VPS — it pollutes all sessions.
- **Stale sessions don't get identity**: Sessions created before the identity system was added won't have env vars or gitconfig set. Must `vpscli end <name>` and recreate.
...
- **No global git identity on VPS**: Git identity is per-session via developer env files. Running git commands outside a session (e.g. bare SSH) requires sourcing `~/.vpscli/developers/<name>.env` first.
...
- **`isOnVPS()` detection**: Uses `existsSync("~/.vpscli")`. When true, SSH functions run commands locally via `bash -c` instead of over SSH, and `tmux switch-client` is used instead of `attach-session`.
...
2. **Do NOT manually bump `VPSCLI_VERSION`** — CI auto-increments it on push to main.
...
8. If server files changed, deploy directly via `scp` (avoids GitHub CDN caching): `scp server/session.sh dev@<host>:~/.vpscli/session.sh` and `scp server/tmux.conf dev@<host>:~/.vpscli/tmux.conf`. For full re-provisioning: `scp -r server/ dev@<host>:/tmp/dev-vps-server && ssh dev@<host> "sudo bash /tmp/dev-vps-server/setup.sh"`
9. If client files changed, users run `vpscli update` (they'll be warned automatically by the version check).

### Testing

- **Syntax check**: `bash -n server/setup.sh && bash -n server/session.sh && bash -n client/vpscli && bash -n client/setup.sh`
...
- **Session test** (needs terminal): `ssh -t dev@<host> "bash ~/.vpscli/session.sh start test-session shell 'test' don"` then `Ctrl+B, D` to detach, `bash ~/.vpscli/session.sh end test-session` to clean up.
- **Non-interactive test**: `ssh dev@<host> 'source ~/.vpscli/developers/don.env && echo "$GIT_AUTHOR_NAME" && git clone https://github.com/kapitolph/ubuntu-v3.git /tmp/test && rm -rf /tmp/test'`
...
3. Commit and push. Users pick it up via `vpscli update`.
